...
async def _update_queue_message(channel_id: int, header: str = ""):
    ...
    if existing_msg:
        try:
            ...
            log.warning(
                f"[queue_msg] Không thể edit tin nhắn cũ: {e}. Sẽ gửi tin nhắn mới."
            )
...
def _play_after(channel_id: int, error):
    ...
    if channel_id in _skip_requests:
        log.debug(
            f"[play] Bỏ qua callback after do lệnh skip đang xử lý ở kênh {channel_id}."
        )
...
async def _play_next_track(channel_id: int):
    ...
    try:
        source = discord.FFmpegPCMAudio(
            next_track["audio_url"], **music_service.ffmpeg_options
        )
        ...
        await _update_queue_message(
            channel_id, header=f"▶️ Đang phát tiếp: **{next_track['title']}**"
        )
    except Exception as e:
            ...
            for voice_client in list(bot.voice_clients):
                ...
                if len(active_users) == 0:
                    ...
                    if voice_client.channel.id not in _alone_since:
                        ...
                        log.info(
                            f"[voice_alone] Bot ở một mình trong kênh {voice_client.channel.id}. Bắt đầu đếm ngược 5s..."
                        )
                    else:
                        elapsed = (
                            time.monotonic() - _alone_since[voice_client.channel.id]
                        )
                        if elapsed >= 5.0:
                            log.info(
                                f"[voice_alone] Đã ở một mình quá 5s tại kênh {voice_client.channel.id}. Tự động rời voice."
                            )
                    ...
                    if voice_client.channel.id in _alone_since:
                        log.info(
                            f"[voice_alone] Có người tham gia lại kênh {voice_client.channel.id}. Hủy đếm ngược."
                        )
                        _alone_since.pop(voice_client.channel.id, None)
        except Exception as e:
...
async def _apply_channel_rate_limit(channel_id: int):
    ...
    if last_fetch is not None:
        ...
        if elapsed < _CHANNEL_RATE_LIMIT_SECONDS:
            ...
            log.info(
                f"[rate_limit] Channel {channel_id}: chờ {wait:.1f}s trước khi fetch..."
            )
        ...
        if result.content_type == "video":
            ...
            for i in range(0, len(result.image_paths), batch_size):
                ...
                if i + batch_size < len(result.image_paths):
                    await asyncio.sleep(
                        random.uniform(_SEND_JITTER_MIN, _SEND_JITTER_MAX)
                    )

        await message.remove_reaction("⏳", bot.user)
        await message.add_reaction("✅")
        log.info(
            f"[tiktok] Hoàn thành gửi {result.content_type} cho channel {message.channel.id}"
        )
...
async def tomtat(ctx, *args):
    ...
    log.info(
        f"[tomtat] Yêu cầu tóm tắt {n} tin nhắn | target={target_str} | channel_id={ctx.channel.id}"
    )
    ...
    async for msg in ctx.channel.history(limit=fetch_limit):
        ...
        if not msg.content.strip():
            log.debug(
                f"[tomtat] Bỏ qua tin nhắn không có text từ {msg.author.name} (chỉ có attachment/sticker)"
            )
            skipped += 1
            continue
    ...
    log.info(
        f"[tomtat] Đã thu thập được {collected} tin nhắn text – đang gọi Gemini API..."
    )
...
async def tomtat_time(ctx, *args):
    ...
    log.info(
        f"[tomtat_time] Yêu cầu tóm tắt {hours} giờ | target={target_str} | channel_id={ctx.channel.id}"
    )
    ...
    async for msg in ctx.channel.history(limit=None):
        ...
        if not msg.content.strip():
            log.debug(
                f"[tomtat_time] Bỏ qua tin nhắn không có text từ {msg.author.name}"
            )
            skipped += 1
            continue
        # Bỏ qua tin nhắn là lệnh (bắt đầu bằng prefix)
        if msg.content.strip().startswith(bot.command_prefix):
            log.debug(
                f"[tomtat_time] Bỏ qua lệnh từ {msg.author.name}: {msg.content[:30]}"
            )
            skipped += 1
            continue
        messages.append(f"{msg.author.name}: {msg.content}")
    ...
    collected = len(messages)
    log.info(
        f"[tomtat_time] Đã thu thập được {collected} tin nhắn text – đang gọi Gemini API..."
    )
    ...
    log.info(
        f"[tomtat_time] Gemini trả về kết quả ({len(summary)} ký tự) – đang gửi..."
    )
    await send_long(ctx, f"**Tóm tắt:**\n{summary}")
...
async def cau_hoi(ctx, *args):
    ...
    async for msg in ctx.channel.history(limit=None):
        ...
        if not msg.content.strip() or msg.content.strip().startswith(
            bot.command_prefix
        ):
            continue
...
async def get_luck(ctx):
    ...
    log.info(
        f"[get_luck] Yêu cầu roll vận may | author={ctx.author} | channel_id={ctx.channel.id}"
    )
    ...
    log.info(
        f"[get_luck] Thu thập {len(user_messages)} tin nhắn của {ctx.author} để gửi Gemini."
    )
...
async def taixiu(ctx):
    ...
    log.info(
        f"[taixiu] Yêu cầu chơi Tài Xỉu | author={ctx.author} | channel_id={ctx.channel.id}"
    )
    ...
    await ctx.send(text)
    log.info(
        f"[taixiu] Hoàn thành roll cho {ctx.author.name}: {result['rolls']} -> {result['result_type']}"
    )
...
async def xinkeo(ctx, *, wish: str = ""):
    ...
    log.info(
        f"[xinkeo] Yêu cầu xin keo | author={ctx.author} | wish='{wish}' | channel_id={ctx.channel.id}"
    )
    if not wish.strip():
        await ctx.send(
    ...
    loop = asyncio.get_event_loop()
    luan_giai = await loop.run_in_executor(
...
async def tarot(ctx, *, question: str = ""):
    ...
    log.info(
        f"[tarot] Yêu cầu xem bói | author={ctx.author} | question='{question}' | channel_id={ctx.channel.id}"
    )
    ...
    drawn_text = (
        ...
        f"🃏 **3 lá bài phụ:** "
        + ", ".join(fmt_card(c) for c in draw_result["supporting_cards"])
        + "\n\n"
...
async def rutque(ctx, *, question: str = ""):
    ...
    log.info(
        f"[rutque] Yêu cầu rút quẻ | author={ctx.author} | question='{question}' | channel_id={ctx.channel.id}"
    )
    if not question.strip():
        await ctx.send(
...
async def luachon(ctx, *, question_and_choices: str = ""):
    ...
    log.info(
        f"[luachon] Yêu cầu lựa chọn | author={ctx.author} | question='{question_and_choices}' | channel_id={ctx.channel.id}"
    )
...
async def thongke_kinhdich(ctx, *, target_str: str = ""):
    ...
    log.info(
        f"[thongke_kinhdich] Yêu cầu thống kê | target='{target_str}' | channel_id={ctx.channel.id}"
    )
    ...
    await _apply_channel_rate_limit(ctx.channel.id)
    display_name = (
        target_str if target_str else (f"@{target_name}" if target_name else "bạn")
    )
    wait_msg = await ctx.send(
    ...
    async for msg in ctx.channel.history(limit=None):
        ...
        if msg.author.id == bot.user.id:
            ...
            if (target_name and target_name in content_lower) or (
                mentioned_id and mentioned_id in msg.content
            ):
    ...
    await wait_msg.edit(
        content=f"Đã tìm thấy {len(history_texts)} lần gieo quẻ/vận may của {display_name} hôm nay. Đang phân tích và luận giải..."
    )
...
async def play(ctx, *, query: str):
    ...
    log.info(
        f"[play] Yêu cầu phát nhạc | query='{query}' | channel_id={ctx.channel.id}"
    )
    ...
    if not voice_client or not voice_client.is_connected():
        ...
        try:
            ...
            await ctx.send(
                f"Không thể kết nối vào cuộc gọi thoại để phát nhạc: {str(e)}"
            )
            return
...
async def next_track(ctx):
    ...
    if not voice_client or not voice_client.is_connected():
        ...
        await ctx.send(
            "❌ Bot không ở trong cuộc gọi thoại nào. Hãy dùng `.play` để phát nhạc trước."
        )
        return
    ...
    if not voice_client.is_playing():
        ...
        await ctx.send(
            "❌ Không có bài hát nào đang phát. Hãy dùng `.play` để phát nhạc trước."
        )
        return
